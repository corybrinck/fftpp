...
public:
  ...
  virtual void fftN(std::complex<T> *pY, size_t ystride,
    ...
    fftB.transform(am.data(), 1, Am.data(), 1, tmp.data(),
                   fftpp::TransformDirection::Inverse);
 ...
 * stride. This can be the same as the input buffer. ystride: Stride of the
 * output buffer in elements pX: Pointer to the start of an input buffer with N
 * elements at the specified stride xstride: Stride of the input buffer in
 * elements pTmp: Pointer to a temporary buffer of length N cols: Number of
 ...
 * false. pY: Pointer to start of a 2D output buffer pX: Pointer to the start of
 * a 2D input buffer rows: Number of rows in the input and output buffers cols:
 * Number of columns in the input and output buffers direction: Direction of the
 * fourier transform. See TransformDirection for details. pFFT: An optional
 * pointer to FFT of length cols. If nullptr or not the needed length a new FFT
 * will be generated internally of the appropriate length. returns: The FFT of
 * length cols that was used to perform the transform
 */
 ...
 * a 2D input buffer rows: Number of rows in the input and output buffers cols:
 * Number of columns in the input and output buffers direction: Direction of the
 * fourier transform. See TransformDirection for details. pPlans: An optional
 * pointer to FFT of length rows. If nullptr or not the correct length a new FFT
 * will be generated internally of the appropriate size. returns: A FFT of
 * length rows that was used to perform the transform
 */
template <typename T, bool RowMajor = true>
 ...
 * Number of columns in the input and output buffers direction: Direction of the
 * fourier transform. See TransformDirection for details. pPlans: An optional
 * pointer to a pair of FFTs where the first element corresponds to the
 * column-wise FFT of length rows and the second element corresponds to the
 * row-wise FFT of length cols returns: The FFTs of length cols and length rows,
 * respectively, used to perform the transforms
 */
template <typename T, bool RowMajor = true>
